...
public class UserFinishView extends EmptyContainer {
    private final VerticalLayout root;
    private final InfoFile infoFile;
    private final UserCreator parent;


    public UserFinishView(UserCreator parent, InfoFile infoFile, double width, double height) {
        super(width, height);
    ...
    private void init() {
        ...
        submit.onClick(event -> {
            ...
            if (parent.getUser() != null) {
                ...
                user = new User(infoFile.get("id"));
            }
            user.setDisplayName(infoFile.get("display-name"));
            user.setPersona(infoFile.getOrDefault("persona", ""));
            ...
            File characterImage = new File(user.getUserDirectory(), "user.png");
            ...
            user.setIconPath(characterImage.getAbsolutePath());
            user.setLorebook(parent.getUserLoreCustomizationView().compileUserLore());
    ...
    private HorizontalLayout buildFinishButton(double width) {
        ...
        root.getPane().addEventHandler(MouseEvent.MOUSE_ENTERED, mouseEvent -> {
            textOverlay.setText("Submit User Changes");
        });
    ...
    private boolean validate() {
        if (parent.getUser() == null && (parent.getUserCustomizationView().getUserIdInput() == null || parent.getUserCustomizationView().getUserIdInput().getCurrentText().isEmpty())) {
            ...
            // Popover
            parent.getUserCustomizationView().getUserIdInput().getNode().requestFocus();
            Popover popover = new Popover(new Text("User id must be provided."));
            popover.setAutoHide(true);
            popover.show(parent.getUserCustomizationView().getUserIdInput().getNode());

            return false;
        }
        if (parent.getUser() == null && App.getInstance().getUser(parent.getUserCustomizationView().getUserIdInput().getCurrentText()) != null) {
            parent.getDisplayContent().removeElement(parent.getDisplayContent().getElements().lastKey());
            parent.getDisplayContent().addElement(parent.getUserCustomizationView());
            parent.setCurrentView(parent.getUserCustomizationView());
            ...
            // Popover
            parent.getUserCustomizationView().getUserIdInput().getNode().requestFocus();
            Popover popover = new Popover(new Text("A user with this id already exists."));
            popover.setAutoHide(true);
            popover.show(parent.getUserCustomizationView().getUserIdInput().getNode());
